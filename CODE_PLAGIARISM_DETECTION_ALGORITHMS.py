...
#############################################################################################################################
def metrics(test_address,reference_address):
	file = open(test_address,'r')
	temp = file.read()
	test_strings=temp.split('\n')
	test_tokens = nltk.word_tokenize(temp)
	file = open(reference_address,'r')
	temp = file.read()
	reference_strings=temp.split('\n')
	reference_tokens = nltk.word_tokenize(temp)
	lines=[]
	test_set= set()
	reference_set = set()
	word_set = set()
	...
	for i in range(len(test_tokens)):
		test_set.add((test_tokens[i]))
	for i in range(len(reference_tokens)):
		reference_set.add((reference_tokens[i]))
	...
	for i in range(len(test_strings)):
		matches=0
		token = nltk.word_tokenize(test_strings[i])
		if len(token)!=0:
			for j in range(len(token)):
				for k in word_set:
					if k == token[j]:
						matches+=1
			similarity=float(matches/len(token))
			if similarity >=0.80:
				test_lines.append(i)

	for i in range(len(reference_strings)):
		matches=0
		token = nltk.word_tokenize(reference_strings[i])
		if len(token)!=0:
			for j in range(len(token)):
				for k in word_set:
					if k == token[j]:
			...
			similarity=float(matches/len(token))
			if similarity >=0.80:
				reference_lines.append(i)

	lines.append(test_lines)
	lines.append(reference_lines)
